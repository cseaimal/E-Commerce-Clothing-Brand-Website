...
export default function ProductCatalog({ navigate }) {
  ...
  // Fetch products when `query` changes with graceful fallback
  useEffect(() => {
    let active = true;
    const fetchProducts = async () => {
      ...
      try {
        ...
        // fallback to sample products when API unavailable
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => setQuery(searchTerm.trim()), 300);
    return () => {
      if (debounceRef.current) clearTimeout(debounceRef.current);
    };
  }, [searchTerm]);
